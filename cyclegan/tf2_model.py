...
class CycleGAN(object):
    ...
    def train(self, args):
        ...
        assert len(dataA) > 0, f"There was a problem loading data A from {self.dataset_A_dir}"
        dataB = glob('{}/train/*.*'.format(self.dataset_B_dir))
        assert len(dataB) > 0, f"There was a problem loading data B from {self.dataset_B_dir}"
